...
def plot_throughput_vs_latency(config_dir='config/postgres/vmsched'):
    # Set today's date for identifying relevant files
    today = datetime.now().strftime("%Y%m%d")
    summary_files = glob.glob(f"{config_dir}/summary_*_{today}.csv")
    ...
    for summary_file in summary_files:
        ...
        with open(summary_file, mode='r') as csv_file:
            ...
            for row in reader:
                # Collect each row's data into a list for sorting
                records.append({
                    "throughput": float(row["Throughput (requests/second)"]),
                    "latency_99th": float(row["99th Percentile Latency (microseconds)"])
                })

        # Sort records by throughput
        records.sort(key=lambda x: x["throughput"])

        # Extract sorted throughput and latency_99th for plotting
        throughput = [record["throughput"] for record in records]
        latency_99th = [record["latency_99th"] for record in records]

        # Store data for plotting
        data[vm_setting] = (throughput, latency_99th)
        print(f"Processed {summary_file} for vm_setting {vm_setting}")

    # Plot each vm_setting as a separate line
    plt.figure(figsize=(10, 6))
    for vm_setting, (throughput, latency_99th) in data.items():
        plt.plot(throughput, latency_99th, marker='o', label=f"VM Setting: {vm_setting}")

    # Label the axes and add title
    plt.xlabel("Throughput (requests/second)")
    plt.ylabel("99th Percentile Latency (microseconds)")
    plt.title("Throughput vs. 99th Percentile Latency")
    plt.legend(title="VM Settings")
    plt.grid(True)

    # Save the plot to a PNG file
    output_file = f"{config_dir}/throughput_vs_latency_{today}.png"
    plt.tight_layout()
    plt.savefig(output_file, format='png')
    print(f"Plot saved as {output_file}")
